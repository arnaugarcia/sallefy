...
public class UserResourceIT {
    ...
    @Test
    @Transactional
    public void getUserPlaylistsTracks() throws Exception {

        // Initialize the database
        User user = userRepository.save(createBasicUserWithUsername("user-playlists-tracks-test"));
        ...
        restUserMockMvc.perform(get("/api/users/{login}", user.getLogin()))
            ...
            .andExpect(jsonPath("$.lastName").value(DEFAULT_LASTNAME))
            .andExpect(jsonPath("$.email").value(DEFAULT_EMAIL))
            .andExpect(jsonPath("$.playlists").value(1))
            .andExpect(jsonPath("$.tracks").value(1))
            .andExpect(jsonPath("$.imageUrl").value(DEFAULT_IMAGEURL))
            .andExpect(jsonPath("$.langKey").value(DEFAULT_LANGKEY));
